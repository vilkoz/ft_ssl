...
for _ in range(NUM_ITER):
    ...
    with open("tmp", "wb") as f:
        f.write(s)
    encoded_text = co("../../ft_ssl des-cbc -e -k "+key+" -v "+iv+" -i tmp", shell=True);
    ssl_encoded = co("openssl des-cbc -e -K "+key+" -iv "+iv+" -in tmp", shell=True);
    # print("encoded_text: ", encoded_text)
    # print(" ssl_encoded: ", ssl_encoded)
    with open("tmp", "wb") as f:
        f.write(ssl_encoded)
    decoded_text = co("../../ft_ssl des-cbc -d -k "+key+" -v "+iv+" -i tmp", shell=True);
    print("try ssl")
    ssl_decoded = co("openssl des-cbc -d -K "+key+" -iv "+iv+" -in tmp", shell=True);
